...
const INITIAL_ENEMY_TAIL_LENGTH: usize = 4;
...
impl Enemy {
    pub fn new(head: Position) -> Self {
        ...
        for i in 1..(INITIAL_ENEMY_TAIL_LENGTH + 1) {
    ...
    pub fn draw(&self, ctx: &Context, g: &mut G2d) {
        for block in self.tail.iter() {
            draw_block(&ctx, g, colors::ENEMY, block)
        }

        draw_enemy_head(&ctx, g, colors::ENEMY, &self.head, &self.direction);
    }
    ...
    pub fn get_len(&self) -> usize {
        &self.tail.len() - INITIAL_ENEMY_TAIL_LENGTH
    }
